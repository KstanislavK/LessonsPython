        ...
        while self.num_of_kegs > 0:
            ...
            if action == 'y' or action == 'н':
                if keg in self.card_p:
                    ...
                    print(f'Вы проиграли матч. Осталось бочонков: {self.num_of_kegs}\nИспользованные бочонки: {self.used_kegs}')
            ...
            if action == 'n' or action == 'т':
                if keg in self.card_p:
                    print(f'Вы проиграли матч. В Вашей карточке есть такое число.\nОсталось бочонков: {self.num_of_kegs}\nИспользованные бочонки: {self.used_kegs}')
                    LeaderBoard().write_in_board()
                    print('Доска почета')
                    LeaderBoard().print_board
                    break
...
class LeaderBoard:
    ...
    def print_board(self):
        ...
        for el in lines:
            el = el.split()
            table += '|' + ' ' * (12 - int(len(el[0])/2)) + el[0] + ' ' * (12 - int(len(el[0])/2)) + '|' + ' '* (12 - int(len(el[1])/2)) + str(el[1]) + ' ' * (12 - int(len(el[1])/2)) + '|\n' + '-' * 52 + '\n'
        print(table)
